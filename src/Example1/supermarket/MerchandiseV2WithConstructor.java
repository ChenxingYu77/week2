...
public class MerchandiseV2WithConstructor {
    ...
    // >> TODO 构造方法（constructor）的方法名必须和类名一样，而且构造方法没有返回值，这样的方法才是构造方法
    // >> TODO 构造方法可以有参数，规则和语法与普通方法一样。使用时，参数传递给new语句后类名的括号后面。
    // >> TODO 如果没有显示地添加一个构造方法，Java会给每个类都会默认自带一个无参数的构造方法。
    // >> TODO 如果我们自己添加类构造方法，Java就不会再添加无参数的构造方法。这时候，就不能直接new一个对象不传递参数了 （看例子）
